...
const validateFormationPayload = ({ titre, date_debut, date_fin, duree, specialite, nb_places, formateur_id, status, prix }) => {
  ...
  if (!duree || Number(duree) <= 0) return 'La durée doit être un nombre positif.';
...
router.post('/formations', (req, res) => {
  ...
  const { titre, description, date_debut, date_fin, duree, specialite, nb_places, formateur_id, prix } = payload;
  ...
  db.query('SELECT id FROM formateurs WHERE id = ?', [formateur_id], (err, formateurs) => {
    ...
    const sql = `
      INSERT INTO formations (titre, description, date_debut, date_fin, duree, specialite, nb_places, prix, formateur_id, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')
    `;
    db.query(sql, [titre, description, date_debut, date_fin, duree, specialite, nb_places, prix || 0, formateur_id], (insertErr, result) => {
...
router.put('/formations/:id', (req, res) => {
  ...
  const { titre, description, date_debut, date_fin, duree, specialite, nb_places, formateur_id, status, prix } = payload;
  ...
  db.query('SELECT id FROM formateurs WHERE id = ?', [formateur_id], (err, formateurs) => {
    ...
    const sql = `
      UPDATE formations
      SET titre=?, description=?, date_debut=?, date_fin=?, duree=?,
          specialite=?, nb_places=?, prix=?, formateur_id=?, status=?
      WHERE id=?
    `;
    db.query(sql, [titre, description, date_debut, date_fin, duree, specialite, nb_places, prix || 0, formateur_id, status, id], (updateErr) => {
...
/* ========================= PUBLISH DIRECT (draft → published) ========================= */
router.put('/formations/:id/publish', (req, res) => {
  db.query("UPDATE formations SET status='published' WHERE id=?", [req.params.id], (err) => {
    if (err) return res.status(500).json({ error: err });
...
router.get('/formateurs', (req, res) => {
  const sql = `
    SELECT f.id, u.nom, u.email, f.specialite
    FROM formateurs f
...
router.post('/formateurs', async (req, res) => {
  ...
  if (!nom || !email || !mot_de_passe) return res.status(400).json({ message: 'Nom, email et mot de passe sont obligatoires.' });
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], async (err, users) => {
    ...
    db.query('INSERT INTO users (nom, email, mot_de_passe, role) VALUES (?, ?, ?, ?)', [nom, email, hashedPassword, 'formateur'], (insertErr, result) => {
      if (insertErr) return res.status(500).json({ error: insertErr });
      db.query('INSERT INTO formateurs (user_id, specialite) VALUES (?, ?)', [result.insertId, specialite || null], (formateurErr) => {
...
router.put('/formateurs/:id', async (req, res) => {
  const { id } = req.params;
  const { nom, email, mot_de_passe, specialite } = req.body;
  if (!nom || !email) return res.status(400).json({ message: 'Nom et email sont obligatoires.' });
  ...
  db.query('SELECT user_id FROM formateurs WHERE id = ?', [id], async (err, results) => {
    ...
    db.query('SELECT * FROM users WHERE email = ? AND id <> ?', [email, userId], (emailErr, existing) => {
      ...
      const updateFormateurUser = (hashedPassword) => {
        ...
        const params = mot_de_passe ? [nom, email, hashedPassword, userId] : [nom, email, userId];
        db.query(userSql, params, (userErr) => {
          if (userErr) return res.status(500).json({ error: userErr });
          db.query('UPDATE formateurs SET specialite=? WHERE id=?', [specialite || null, id], (formateurErr) => {
            if (formateurErr) return res.status(500).json({ error: formateurErr });
...
router.put('/formations/:id/programme', (req, res) => {
  const { id } = req.params;
  ...
  `;
  db.query(sql, [id, description_globale || null, objectifs || null, prerequis || null], (err) => {
    if (err) return res.status(500).json({ error: err });
    res.json({ message: 'Programme sauvegardé.' });
  });
});

router.post('/formations/:id/modules', (req, res) => {
  const { id } = req.params;
...
router.put('/formations/:formationId/modules/:moduleId', (req, res) => {
...
router.delete('/formations/:formationId/modules/:moduleId', (req, res) => {
  const { moduleId } = req.params;
  db.query('DELETE FROM modules_formation WHERE id=?', [moduleId], (err) => {
    if (err) return res.status(500).json({ error: err });
